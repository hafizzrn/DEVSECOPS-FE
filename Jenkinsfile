pipeline {
    ...
    environment {
        IMAGE_NAME = 'secure-cashflow-fe-app'
        STAGING_PORT = '3001'
        PROD_PORT = '3000'
        INTERNAL_PORT = '3001'
        SONAR_PROJECT_KEY = 'secure-cashflow-fe-app'
    }
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/hafizzrn/DEVSECOPS-FE.git', branch: 'main'
            }
        }
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        cat > .env <<EOF
NEXT_PUBLIC_BASE_URL=https://api-cashflow-secure.nflrmvs.cloud/api/v1
...
services:
  nextjs:
    ...
    container_name: cashflow-fe-staging
    ports:
                ...
                script {
                    ...
                    sh """
                        ...
                        docker run --rm -u 0 \
                          ...
                          -t http://cashflow-fe-staging:${INTERNAL_PORT} \\
                          -r zap-report.html \\
                          -w zap-report.md \\
                          -J zap-report.json \\
                          -I
...
NEXTAUTH_URL=https://cashflow-secure.nflrmvs.cloud
NODE_ENV=production
APP_PORT=${PROD_PORT}
EOF
